...
if 1:
    ...
    #        for solver in ['sim','sim1d','newton']:   #,'newtonstack']:
        ...
        # for solver in ['sim']:
            with model.timer(f'Solve with:{solver:26}',short=True)  as t:
                _ = msmec(muldf,2020,2200,solver=solver,max_iterations=400,
                          ljit=turbo,chunk=30,
                          ldumpvar=True,dumpvar='*',
                          conv='fy*',first_test=20,alfa=.4,
                          relconv=0.0000000000001,
                          ...
                          keep_solution=f'Experiment "TG + {tg_plus}":',keep_variables='*'
                               )
...
with msmec.set_smpl(2017,2200):        
    msmec.keep_plot('ul enl fy',legend=0,diff=0,growth=0,scale='linear',mul=1,dec='')
