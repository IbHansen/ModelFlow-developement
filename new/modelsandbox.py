...
class newmodel(model):
    ...
    def __call__(self, *args, **kwargs ):
            ...
            if self.maxlead >= 1:
                outdf = self.newtonstack(*args, **newkwargs )   
...
#                self.pronew2d,self.solvenew2d,self.epinew2d  = make_los(self.funks,self.errfunk)
        ...
        if not hasattr(self,'newton_diff'):
            ...
            self.newton_diff = new_newton_diff(self,forcenum=1,df=databank,
        ...
        #                    print(f'Iteration {iteration} sum of distances {newton_conv}')
                ...
                #            update = self.solveinv(distance)
                            ...
                            # breakpoint()
                            if not silent : print(f'{iteration} Distance\n{distance}\nUpdate\n{update}')
...
#                self.prostack2d,self.solvestack2d,self.epistack2d  = make_los(self.funks,self.errfunk)
        ...
        if not hasattr(self,'newton_diff_stack'):
            self.newton_diff_stack = new_newton_diff(self,forcenum=forcenum,df=databank,ljit=nljit,nchunk=nchunk,timeit=timeit)
...
class newton_diff():
    ...
    def __init__(self, mmodel, df = None , endovar = None,onlyendocur=False, 
        ...
        self.declared_endo_list =  [pt.kw_frml_name(self.mmodel.allvar[v]['frmlname'], 'ENDO') 
           for v in self.mmodel.endogene if pt.kw_frml_name(self.mmodel.allvar[v]['frmlname'], 'ENDO')]
    ...
    def modeldiff(self):
        ...
        def findallvar(model,v):
            ...
            if self.onlyendocur :
                rhsvar={(nt.var+('('+nt.lag+')' if nt.lag != '' else '')) for nt in terms if nt.var and nt.lag == '' and nt.var in self.endovar or nt.var in self.declared_endo_set}
                
            else:
                rhsvar={(nt.var+('('+nt.lag+')' if nt.lag != '' else '')) for nt in terms if nt.var and nt.var in self.endovar or nt.var in self.declared_endo_set}
...
class new_newton_diff():
    ...
    def modeldiff(self):
        ...
        def findallvar(model,v):
            ...
            terms= self.mmodel.allvar[v]['terms'][model.allvar[v]['assigpos']:-1]
            if self.onlyendocur :
                rhsvar={(nt.var+('('+nt.lag+')' if nt.lag != '' else '')) for nt in terms if nt.var and nt.lag == '' and nt.var in self.declared_endo_set}
                
            else:
                rhsvar={(nt.var+('('+nt.lag+')' if nt.lag != '' else '')) for nt in terms if nt.var and nt.var in self.declared_endo_set}
            var2=sorted(list(rhsvar))
            return var2
    ...
    def get_diff_mat_tot(self,periode='',df=None):
        ...
        with ttimer('Prepare wide input to sparse matrix',self.timeit):
            ...
            self.coltupnum = [(self.placdic[var],self.placdic[pvar+'___RES'],lag) 
                               for var,pvar,lag in self.coltup]
        ...
        with ttimer('melt the wide input to sparse matrix',self.timeit):
            self.dmelt = self.difres.melt(id_vars='number')
            self.dmelt.loc[:,'value']=self.dmelt['value'].astype('float')
    ...
    #csc_matrix((data, (row_ind, col_ind)), [shape=(M, N)]) 
            values =  self.dmelt.value.values 
            indicies = self.dmelt.row.values
            indicies = (self.dmelt.row,self.dmelt.col)
    ...
    def get_diff_mat_1per(self,periode='',df=None):
        ...
        with ttimer('calculate derivatives',self.timeit):
            self.difres = self.diff_model.res2d(_df,silent=self.silent,stats=0,ljit=self.ljit,chunk=self.nchunk).loc[_per,self.diff_model.endogene]
            self.difres_before = self.difres.copy()
        with ttimer('Prepare wide input to sparse matrix',self.timeit):
...
#        if update or not hasattr(self,'stacked'):
    ...
    def get_jacdf(self,df='',periode=''):
        self.jacsparsedic = self.get_diff_mat_1per(df=df,periode=periode)
        self.jacdfdic = {p: pd.DataFrame(jac.toarray(),columns=self.declared_endo_list,index=self.endovar) for p,jac in self.jacsparsedic.items()}
        return self.jacdfdic
...
#%%        
        ...
        mat_dif = nn.get_diff_mat_tot(df2)
        md1 = mat_dif.toarray()
 #%%       
